...
if($id != ""){
    $sql1   = "select * from microcontroller where id = '$id'";
...
if (isset($_POST['simpan'])) {
    ...
    //Array ( [foto] => Array ( [name] => .....jpg [type] => image/jpeg [tmp_name] => C:\xampp2\tmp\php4FDD.tmp [error] => 0 [size] => 2375701 ) )
    // print_r($_FILES);
    if($_FILES['foto']['name']){
        ...
        // Array ( [dirname] => . [basename] => ......jpg [extension] => jpg [filename] => ..... )
    ...
    if (empty($error)) {
        if($foto_name){
            ...
            $foto_name = "microcontroller_".time()."_".$foto_name;
        ...
        if($id != ""){
            $sql1   = "update microcontroller set nama = '$nama',foto='$foto_name',isi='$isi',tgl_isi=now() where id = '$id'";
        }else{
            $sql1       = "insert into microcontroller(nama,foto,isi) values ('$nama','$foto_name','$isi')";
        }
...
?>
<h1>Halaman Admin Input Data Microcontroller</h1>
<div class="mb-3 row">
    <a href="microcontroller.php">
        << Kembali ke halaman admin Microcontroller</a>
</div>
<?php
if ($error) {
?>
